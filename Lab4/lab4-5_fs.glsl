...
void init( )
{
    ...
    // Positions of the boxes in the Minecraft-style structure
    vec3 box_positions[NUM_BOXES];

    // Bottom layer: left side
    box_positions[0] = vec3(3.0, 0.0,  0.5);
    box_positions[1] = vec3(3.0, 0.0,  0.0);
    box_positions[2] = vec3(3.0, 0.0, -0.5);
    box_positions[3] = vec3(3.0, 0.0, -1.0);

    // Bottom layer: right side
    box_positions[4] = vec3(4.5, 0.0,  0.5);
    box_positions[5] = vec3(4.5, 0.0,  0.0);
    box_positions[6] = vec3(4.5, 0.0, -0.5);
    box_positions[7] = vec3(4.5, 0.0, -1.0);

    // Bottom layer: back and centre
    box_positions[8]  = vec3(3.5, 0.0, -1.0);
    box_positions[9]  = vec3(4.0, 0.0, -1.0);
    box_positions[10] = vec3(3.5, 0.0, -0.5);
    box_positions[11] = vec3(4.0, 0.0, -0.5);

    // Upper layer
    box_positions[12] = vec3(3.0, 0.5, -1.0);
    box_positions[13] = vec3(3.0, 0.5, -0.5);
    box_positions[14] = vec3(3.5, 0.5, -1.0);
    box_positions[15] = vec3(4.0, 0.5, -1.0);
    box_positions[16] = vec3(4.5, 0.5, -1.0);
    box_positions[17] = vec3(4.5, 0.5, -0.5);

    // Material for all boxes
    Material box_material;
    box_material.color_diffuse = vec3(0.4, 0.7, 0.1);
    box_material.color_glossy = vec3(1.0);
    box_material.color_emission = vec3(0.0);
    box_material.roughness = 100.0;
    box_material.reflection = 0.0;
    box_material.transmission = 0.0;
    box_material.ior = 1.0;

    // Create equally sized cubes at the positions above
    for (int i = 0; i < NUM_BOXES; ++i)
    {
      scene.boxes[i].minimum = box_positions[i];
      scene.boxes[i].maximum = box_positions[i] + vec3(0.48);
      scene.boxes[i].material = box_material;
    }
}
...
// AABB slab intersection adapted from Majercik et al. (2018), Listing 1. Modified to return the nearest positive intersection distance.
float intersect(Ray ray, Box box)
{
  vec3 inverse_direction = 1.0 / ray.dir;

  vec3 t0 = (box.minimum - ray.origin) * inverse_direction;
  vec3 t1 = (box.maximum - ray.origin) * inverse_direction;

  vec3 t_near = min(t0, t1);
  vec3 t_far = max(t0, t1);

  float near_distance = max(max(t_near.x, t_near.y), t_near.z);
  float far_distance = min(min(t_far.x, t_far.y), t_far.z);

  if (near_distance > far_distance || far_distance <= 0.0)
    return -1.0;
...
Intersection intersect( Ray ray)
{
  ...
  for (int i = 0; i < NUM_BOXES; ++i)
  {
    float d = intersect(ray, scene.boxes[i]);

    if (d > 0.0 && d <= t)
    {
      t = d;

      // Store the box intersection point and material
      I.point = ray.origin + t * ray.dir;
      I.material = scene.boxes[i].material;

      // Find the centre of the box
      vec3 centre = (scene.boxes[i].minimum + scene.boxes[i].maximum) / 2.0;

      // Direction from the centre to the intersection point
      vec3 v = normalize(I.point - centre);

      // Make the largest component equal to 1 or -1
      float largest = max(max(abs(v.x), abs(v.y)), abs(v.z));
      vec3 w = v / largest;

      // Determine which box face was hit
      if (w.x < -0.99)
...

  {
    ...
    for(int ray_stack_pos=0; ray_stack_pos < ray_stack_size; ++ray_stack_pos)
    {
      ...
      // Now handle non-specular scattering (i.e., the non-recursive case)
      {
        ...
        // Check whether a box blocks the light
        if (!in_shadow)
        {
          for (int i = 0; i < NUM_BOXES; ++i)
          {
            float box_distance = intersect(shadow_ray, scene.boxes[i]);

            if (box_distance > 0.0 && box_distance < light_distance)
            {
              in_shadow = true;
              break;
            }
